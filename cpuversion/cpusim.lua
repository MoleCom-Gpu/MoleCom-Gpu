...
function readConfiguration()
	...
	receiversCoordinates = torch.DoubleTensor(numberOfReceivers, 3)
	transmittersCoordinates = torch.DoubleTensor(numberOfTransmitters, 3)
	receiversRadius = torch.DoubleTensor(numberOfReceivers)
	transmittersRadius = torch.DoubleTensor(numberOfTransmitters)
...
function generateMolecules(transmitterNumber)
	...
	moleculeX = torch.DoubleTensor(1, symbolSize): uniform(-1 * radius, radius)
	moleculeY = torch.DoubleTensor(1, symbolSize): uniform(-1 * radius, radius)
	-- if x^2 + y^2 >= r^2, set y^2 = r^2 - x^2 (This might be changed in order to improve randomness)
        ...
        flag = ((sqMoleculeX + sqMoleculeY - radius * radius):ge(0)):double()
	...
	-- now randomly set sign of z coordinate (by square root only positive values are retrieved but the occurance of positive and negative values are equally likely. 
        signGenerator = torch.DoubleTensor(1, symbolSize): uniform(-1, 1)
        signGenerator = signGenerator:ge(0):double()
...
receiverCount = torch.DoubleTensor(numberOfReceivers, numberOfSymbols): fill(0) -- it might be also a double tensor
-- generate first iteration of molecules
-- to do concat. first initialize molecules
molecules = generateMolecules(1)
availability = torch.DoubleTensor(symbolSize): fill(1)
-- then do for other transmitters.
for i=2, numberOfTransmitters do
	molecules = torch.cat(molecules, generateMolecules(i))
	availability = torch.cat(availability, torch.DoubleTensor(symbolSize): fill(1))
end
for i = 1, loopLength do
	if (i - 1) % symbolCheck == 0 and i > 1 then -- If there is time to generate new symbol and evaluate the previous one
		...
		for i=1, numberOfTransmitters do
        		molecules = torch.cat(molecules, generateMolecules(i))
        		availability = torch.cat(availability:double(), torch.DoubleTensor(symbolSize): fill(1))
     		end
	...
	delta1 = torch.DoubleTensor(numberOfMolecules): normal(0, twoDT)
	delta2 = torch.DoubleTensor(numberOfMolecules): normal(0, twoDT)  
	delta3 = torch.DoubleTensor(numberOfMolecules): normal(0, twoDT) 
	molecules[1]: add(delta1:cmul(availability:double()))
	molecules[2]: add(delta2:cmul(availability:double()))
	molecules[3]: add(delta3:cmul(availability:double()))
